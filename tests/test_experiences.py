...
from functionrl.experiences import generate_episode, generate_episodes, generate_experiences
...
def test_episode_gen(env, policy):
    episodes = list(generate_episodes(env, policy, n=4))
    assert [len(episode) for episode in episodes] == [5, 2, 2, 3]


def test_generate_experiences(env, policy):
    experiences = list(generate_experiences(env, policy, n=10))
